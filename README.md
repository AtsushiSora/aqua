...
- 通知配信ログからの本番送信結果反映
- 期限切れPush購読の自動無効化
- ホームのデザイン変更ボタンとアカウント画面での表示モード切り替え（ベーシックモード / かんたんモード / 一目モード / 大人モード / ライブモード）
...
- かんたん/一目/大人モードのホーム操作サイズと密度を差別化
- ライブモードでトップ写真にゆらぎ、光、ゆっくりズームを重ねる動的演出
- PWA実機テストでの5モード/画像カスタムQA補助表示
...
5. ベーシック、かんたん、一目、大人、ライブモードを切り替え、ホームで主要操作を試す。
   ライブモードでは、水面のゆらぎ、トップ写真のゆっくりズーム、横スクロールなし、端末の「動きを減らす」設定で過度な動きが止まることも確認します。
...
ログイン、ホーム追加、通知受信、オフライン復帰、5モード表示、画像カスタムのOK状況はレビューサマリーで確認できます。
...
Supabase SQL確認、Storage確認、AI Gateway確認は本番前セットアップの確認メモに保存でき、PWAレビューJSONにも確認状態として含まれます。
...
1. 本番URLを実機で開き、ログイン、ホーム追加、通知受信、オフライン復帰、5モード表示、画像カスタムを確認する。
...
ライブモード保存時に既存制約が残っている場合は、アプリ上でもこのSQL実行が必要なことを案内します。
