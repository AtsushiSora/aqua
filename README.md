...
- アカウント画面の状態サマリーと基本情報、デザイン、通知、ログインへのショートカット
- プロフィールのユーザーID（公開用）に用途説明を表示
- 未ログイン時の登録案内ポップと、新規登録フォームへの誘導
- モニター画面で、配布準備、参加者、感想・不具合メモを独立管理
- モニター画面で、参加者へ送る本番URLの表示とコピー
- モニター参加者向けの登録手順と試してほしい操作の説明、コピー用送付文
- アカウント画面はアカウント情報に整理し、PWA確認、本番セットアップ、配信ログ、データ移行は管理者画面へ分離
...
- PWA実機テスト項目ごとのメモ例入力
- PWA実機テストの端末/ブラウザプリセット入力
- PWA実機テストの次に確認する項目入力導線
- PWA実機QA完了後の最終リリース判定導線
...
- PWA最終リリース判定からモニター配布セットへ進む導線
- モニター参加者へ送る案内文のコピー機能
- モニター参加者へ送る返信テンプレート付き案内文
- モニター参加者向け案内文の初心者向け文面調整
- モニター配布セットJSONの書き出しと準備チェック
- 本番URL未入力時のモニター配布セット書き出し防止
- 配布セット書き出し前のモニター案内文コピー防止
- モニター開始準備の進捗率と残り件数表示
- モニター開始OK時の案内文コピー導線
- モニター開始直前の停止/目視確認/開始OK判定
- モニター開始前チェックから不足項目の入力場所へ移動する導線
- モニター配布セットのURL/送付文/返信テンプレート事前レビュー
- モニター参加者へ送る全文プレビューの開閉表示
- モニター参加者向け案内文と再送文のスマホ返信向け短縮
- 職場モニター2人のクイック追加
- 職場モニター2人の一括送付済み更新
- モニター参加者リストと送付/返信状況管理
- モニター未返信者への再送文コピー
- モニター指摘の公開前に直す/あとで直す/対応済み判断メモ
- PWA最終リリース判定でのモニター指摘OK/あとで直す件数表示
- PWA公開OK保存前チェックの未完了件数表示
- モニターフィードバック入力への返信テンプレート挿入
- モニター返信テンプレートから端末/画面/分類をフォームへ反映
- モニター返信テンプレートから参加者名をフォームへ反映
- モニター感想入力フォームの初心者向け短縮表示
- モニターフィードバックフォームの保存前不足チェック
- モニターフィードバック保存後の次候補/修正リスト導線
- モニターフィードバックの今日直す候補リスト表示
- モニターフィードバックの参加者数/端末数の回収状況表示
- モニターフィードバックの最低回収目安 2人/2端末 判定
- モニターフィードバックの次対応候補表示とJSONトリアージ
- モニターフィードバックの優先修正リストコピー
- モニターフィードバックのCSV書き出し
...
1. モニター画面の「モニター版チェック」で優先タスクを確認する。
2. PWA最終リリース判定に本番URL `https://apuanote.netlify.app/#dashboard` を入れて保存する。URL未入力のままでは「配布セットJSON」は書き出せません。
3. 「配布セットJSON」を書き出し、モニターURL、案内文、返信テンプレートを確認する。
4. 「案内文をコピー」で参加者へ送る文面をコピーし、2人以上に共有する。
5. 返信が来たら「返信テンプレートを入れる」または返信本文を貼り、「返信内容を反映」で端末、画面、分類、優先度を補完する。
6. 2人以上、2端末以上の回収状況を確認する。
7. 未対応、高優先度、不具合/UI指摘を上から確認し、対応済みにしたら対応メモを残す。
8. 「修正リスト」で今日直す候補をコピーし、CSV/JSONを書き出してモニター中に直す候補と対応済みの証跡を残す。
